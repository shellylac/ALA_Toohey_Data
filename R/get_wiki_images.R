...
safe_get_infobox_image <- purrr::possibly(get_infobox_image, otherwise = NA_character_)
...
image_urls_df <- data.frame(wiki_url = urls, image_url = image_urls) |>
  mutate(image_url = dplyr::if_else(wiki_url == "https://en.wikipedia.org/wiki/Ctenotus_spaldingi",
                                     "https://www.jcu.edu.au/__data/assets/image/0010/97372/366939.3.jpg",
                                     image_url)
         )

readr::write_rds(image_urls_df,
