...
get_infobox_data <- function(url) {
  tryCatch({
    ...
    response <- httr::GET(url, httr::user_agent("Mozilla/5.0 (Windows NT 10.0; Win64; x64)"))
    ...
    iucn_node <- page |> rvest::html_node(xpath = "/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table/tbody/tr[4]/td/div/a")
    # Alternative path seen for welcome swallow
    if (is.na(iucn_node)) {
      iucn_node <- page |> rvest::html_node(xpath = "/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table/tbody/tr[5]/td/div/a")
    }
    if (is.na(iucn_node)) {
      iucn_node <- page |> rvest::html_node(xpath = "/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table/tbody/tr[6]/td/div/a")
    }
    if (is.na(iucn_node)) {
      iucn_node <- page |> rvest::html_node(xpath = "/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table/tbody/tr[7]/td/div/a")
    }
    if (is.na(iucn_node)) {
      iucn_node <- page |> rvest::html_node(xpath = "/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table/tbody/tr[3]/td/div/a")
    }
    if (is.na(iucn_node)) {
      iucn_node <- page |> rvest::html_node(xpath = "/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table[1]/tbody/tr[4]/td/div")
    }
...
safe_get_infobox_data <- purrr::possibly(get_infobox_data,
                                         otherwise = list(image_url = NA_character_,
                                                          iucn_status = NA_character_))
